/* Target-dependent code for OS/161 on MIPS, for GDB,
   the GNU Debugger. Based on mipsnbsd-tdep.c.
...
/* Number of registers in `struct reg' from netbsd <machine/reg.h>.  */
#define MIPS_OS161_NUM_GREGS	38

/* Number of registers in `struct fpreg' from netbsd <machine/reg.h>.  */
#define MIPS_OS161_NUM_FPREGS	33
...
static void
mips_os161_supply_fpregset (const struct regset *regset,
			    ...
			    int regnum, const void *fpregs, size_t len)
{
  ...
  gdb_assert (len >= MIPS_OS161_NUM_FPREGS * regsize);
...
static void
mips_os161_supply_gregset (const struct regset *regset,
			   ...
			   const void *gregs, size_t len)
{
  ...
  gdb_assert (len >= MIPS_OS161_NUM_GREGS * regsize);
  ...
  if (len >= (MIPS_OS161_NUM_GREGS + MIPS_OS161_NUM_FPREGS) * regsize)
    {
      regs += MIPS_OS161_NUM_GREGS * regsize;
      len -= MIPS_OS161_NUM_GREGS * regsize;
      mips_os161_supply_fpregset (regset, regcache, regnum, regs, len);
    }
}

/* NetBSD/mips register sets.  */

static const struct regset mips_os161_gregset =
{
  NULL,
  mips_os161_supply_gregset
};

static const struct regset mips_os161_fpregset =
{
  NULL,
  mips_os161_supply_fpregset
};
...
mips_os161_regset_from_core_section (struct gdbarch *gdbarch,
				     const char *sect_name, size_t sect_size)
{
  size_t regsize = mips_isa_regsize (gdbarch);
  
  if (strcmp (sect_name, ".reg") == 0
      && sect_size >= MIPS_OS161_NUM_GREGS * regsize)
    return &mips_os161_gregset;

  if (strcmp (sect_name, ".reg2") == 0
      && sect_size >= MIPS_OS161_NUM_FPREGS * regsize)
    return &mips_os161_fpregset;
...
#define OS161_MIPS_JB_ELEMENT_SIZE(gdbarch)	mips_isa_regsize (gdbarch)
#define OS161_MIPS_JB_OFFSET(gdbarch)		(OS161_MIPS_JB_PC * \
					 OS161_MIPS_JB_ELEMENT_SIZE (gdbarch))

static int
mips_os161_get_longjmp_target (struct frame_info *frame, CORE_ADDR *pc)
{
  ...
  buf = alloca (OS161_MIPS_JB_ELEMENT_SIZE (gdbarch));

  jb_addr = get_frame_register_unsigned (frame, MIPS_A0_REGNUM);

  if (target_read_memory (jb_addr + OS161_MIPS_JB_OFFSET (gdbarch), buf,
  			  OS161_MIPS_JB_ELEMENT_SIZE (gdbarch)))
    return 0;

  *pc = extract_unsigned_integer (buf, OS161_MIPS_JB_ELEMENT_SIZE (gdbarch),
				  byte_order);
  return 1;
}

static int
mips_os161_cannot_fetch_register (struct gdbarch *gdbarch, int regno)
{
...
static int
mips_os161_cannot_store_register (struct gdbarch *gdbarch, int regno)
{
  return (regno == MIPS_ZERO_REGNUM
	  || regno == mips_regnum (gdbarch)->fp_implementation_revision);
}
                   ...
                   struct gdbarch *gdbarch)
{
  ...
  set_gdbarch_cannot_fetch_register (gdbarch, mips_os161_cannot_fetch_register);
  set_gdbarch_cannot_store_register (gdbarch, mips_os161_cannot_store_register);

  set_gdbarch_software_single_step (gdbarch, mips_software_single_step);

  /* OS/161 has SVR4-style shared libraries.  */
